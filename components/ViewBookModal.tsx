...
import { Quote as QuoteIcon, Plus } from 'lucide-react';
...
export function ViewBookModal({
  ...
  const loadQuotes = async () => {
    ...
    try {
      const response = await fetch(`/api/books/${book.id}/quotes`);
      if (response.ok) {
  ...
  const handleSaveQuote = async (quoteData: Partial<QuoteData>) => {
    if (!book) return;
  ...
  const handleUpdateQuote = async (quoteId: string, updates: Partial<QuoteData>) => {
    try {
      const response = await fetch(`/api/quotes/${quoteId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fadeIn">
        <div className="bg-white rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" style={{
          ...
          <div className="p-6">
            ...
            {/* Quotes Section */}
            <div className="mt-8 pt-6 border-t-2" style={{ borderColor: '#e5e7eb' }}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <QuoteIcon className="w-6 h-6" style={{ color: '#8b6f47' }} strokeWidth={2.5} />
                  <h3 className="text-2xl font-black" style={{ color: '#5d4e37', fontFamily: 'Playfair Display, serif' }}>
                    Quotes & Highlights
                  </h3>
                  {quotes.length > 0 && (
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 rounded-xl font-bold transition-all duration-300 hover:scale-105 shadow-md"
                  style={{
              ...
              {isLoadingQuotes ? (
                ...
                <div className="text-center py-12 px-4 rounded-2xl" style={{
                  background: 'linear-gradient(135deg, rgba(139, 111, 71, 0.05), rgba(139, 111, 71, 0.1))',
                  border: '2px dashed rgba(139, 111, 71, 0.3)',
                }}>
                  <QuoteIcon className="w-16 h-16 mx-auto mb-4" style={{ color: '#8b6f47', opacity: 0.3 }} strokeWidth={1.5} />
                  <p className="text-gray-600 font-medium">
                    No quotes yet. Start collecting your favorite passages!
                  </p>
                </div>
              ) : (
